...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $price = $_POST['price'];
    $old_price = $_POST['old_price']; // Obtener el valor de old_price del input
    $stock = $_POST['stock'];
    ...
    if ($id) {
        ...
        $query = "
            UPDATE productos 
            SET name = :name, description = :description, characteristics = :characteristics, price = :price, old_price = :old_price, stock = :stock, datasheet = :datasheet, brand_id = :brand_id, category_id = :category_id, discount = :discount, `new` = :is_new, manual = :manual
        ";
        ...
        $stmt->bindParam(':old_price', $old_price); // Bind del old_price
        ...
        $stmt = $conn->prepare("
            INSERT INTO productos (name, description, characteristics, price, old_price, stock, imagen, datasheet, brand_id, category_id, discount, `new`, manual) 
            VALUES (:name, :description, :characteristics, :price, :old_price, :stock, :imagen, :datasheet, :brand_id, :category_id, :discount, :is_new, :manual)
        ");
...
<div class="form-group">
    <label for="old_price">Precio Antiguo:</label>
    <input type="number" class="form-control" step="0.01" name="old_price" id="old_price" value="<?php echo htmlspecialchars($product['old_price'] ?? '', ENT_QUOTES); ?>">
</div>
